...
import PostMessage from "../models/postMessage.js";// give us acess to our model
...
export const getPostsBySearch = async (req, res) => {
    ...
    try {
        ...
        const posts = await PostMessage.find({ $or: [ { title }, { tags: { $in: tags.split(',') } } ]});
            //$or either find me the title or the tags
            //$in is there a tag in tags that matches the query
        res.json({ data: posts });
    } catch (error) {    
        res.status(404).json({ message: error.message });
    }
